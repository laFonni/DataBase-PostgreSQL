...
Sprawdz czy napisana odpowiedz jest poprawna 
Napisz zapytanie SQL pobierające nazwy utworów oraz ich średnią ocenę, będących na playlistach klienta o loginie kamila, które nie otrzymały od niego żadnej oceny. Średnia ocena jest sumą wszystkich wartości ocen (kolumna lubi) podzieloną przez ich liczbę. Przyjmujemy, że wartość TRUE to 1, a
wartość FALSE to 0. Jeżeli utwór nie ma żadnych ocen to zapytanie powinno zwrócić 0.5 jako jego średnią
ocenę
...
swo AS (
	SELECT u.idutworu, AVG(COALESCE(o.lubi::integer, 0.5)) as srednia
	FROM 
		utowry u
		LEFT JOIN oceny o USING(idutworu)
)

SELECT p.nazwa, s.srednia
FROM pkbo p 
	CROSS JOIN swo s USING(idutworu)


SELECT DISTINCT a.gatunek
FROM klienci k
...
WHERE
	k.data_urodzenia = (SELECT data_rejestracji FROM klienci WHERE login = 'jim') 